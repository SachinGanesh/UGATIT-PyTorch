...
if __name__ == '__main__':
    ...
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, pin_memory=False)
    ...
    netG_B2A = Generator(image_size=args.image_size,ngf=image_ch).to(device)
    netD_B = Discriminator(n_layers=5,ndf=image_ch).to(device)
    netD_A = Discriminator(n_layers=5,ndf=image_ch).to(device)
    netL_A = Discriminator(n_layers=7,ndf=image_ch).to(device)
    netL_B = Discriminator(n_layers=7,ndf=image_ch).to(device)
